...
<body class="footer-offset">
        ...
        $(document).ready(function() {
            $('#searchForm input[name="search"]').keyup(function() {
                ...
                if (searchKeyword.length >= 1) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.length === 0) {
                                $('#searchResults').html('<div class="fs-16 fw-500 mb-2">' +
                                    ...
                                    '<img width="30" src="' +
                                    @json(asset('/public/assets/admin/img/no-search-found.png')) + '" alt="">' + ' ' +
                    ...
                    $('#searchResults').html(
                        '<div class="text-center text-muted py-5">{{ translate('Write something to search.') }}.</div>'
                    );
                }
            });
        });
                        ...
                        if (response.length === 0) {
                            ...
                            response.forEach(function(route) {
                                resultHtml += '<a href="' + route.route_full_url +
                                    '" class="search-list-item d-flex flex-column" data-route-name="' +
                                    route.route_name + '" data-route-uri="' + route
                                    .route_uri + '" data-route-full-url="' + route
                                    .route_full_url + '" aria-current="true">';
                            ...
                            $('#searchResults').html(
                                '<div class="recent-search fs-16 fw-500 animate">' +
                                @json(translate('Recent Search')) +
                                '<div class="search-list d-flex flex-column mt-2">' +
                                resultHtml + '</div></div>');
    ...
    <script>
        ...
        function openCodeInput() {
            ...
            $('#manual-code').removeClass('d-none');
