                ...
                <ul class="navbar-nav align-items-center flex-row">
                    <li class="nav-item max-sm-m-0 w-xxl-200px ml-auto mr-2 flex-grow-0">
                        <button type="button" id="modalOpener"
                            class="title-color bg--secondary border-0 rounded justify-content-between w-100 align-items-center py-2 px-2 px-md-3 d-flex gap-1"
                            data-toggle="modal" data-target="#staticBackdrop">
                            <div class="align-items-center d-flex flex-grow-1 gap-1 justify-content-between">
                                <span
                                    class="align-items-center d-none d-xxl-flex gap-2 text-muted">{{ translate('Search_or') }}

                                    <span
                                        class="bg-E7E6E8 border ctrlplusk d-md-block d-none font-bold fs-12 fw-bold lh-1 ms-1 px-1 rounded text-muted">Ctrl+K</span>

                                </span>
                                <img width="14" src="{{ asset('/public/assets/admin/img/new-img/search.svg') }}"
                                    class="svg" alt="">
                            </div>
                        </button>
                    </li>
                        ...
                        <div class="hs-unfold">
                            <a class="js-hs-unfold-invoker navbar-dropdown-account-wrapper" href="javascript:;"
                                ...
                                <div class="cmn--media right-dropdown-icon d-flex align-items-center">
                                    ...
                                    <div class="avatar avatar-sm avatar-circle">
                                        <img class="avatar-img  onerror-image aspect-1-1"
                            ...
                            <div id="accountNavbarDropdown"
                                ...
                                <div class="dropdown-item-text">
                                    <div class="media align-items-center">
                                        ...
                                        <div class="media-body">
                                            <span
                                                class="card-title h5">{{ \App\CentralLogics\Helpers::get_loggedin_user()->f_name }}
                                                {{ \App\CentralLogics\Helpers::get_loggedin_user()->l_name }}</span>
                                            <span
                                                class="card-text">{{ \App\CentralLogics\Helpers::get_loggedin_user()->email }}</span>
                                        </div>
                                    </div>
                                ...
                                <a class="dropdown-item log-out">
                                    <span class="text-truncate pr-2 log-out"
                    ...
                    <form class="flex-grow-1" id="searchForm" action="{{ route('vendor.search.routing') }}">
                        ...
                        <div class="d-flex align-items-center global-search-container">
                            <input autocomplete="off" class="form-control flex-grow-1 rounded-10 search-input"
                                id="searchInput" maxlength="255" name="search" type="search"
                                placeholder="{{ translate('Search_by_keyword') }}" aria-label="Search" autofocus>
                        </div>
                    </form>
                    <div class="position-absolute right-0 pr-2">
                        <button class="border-0 rounded px-2 py-1" type="button"
                            data-dismiss="modal">{{ translate('Esc') }}</button>
                    </div>
                </div>
...
@if ($Payable_Balance == 1 && $cash_in_hand_overflow && $wallet?->balance < 0 && $val <= abs($wallet?->collected_cash))
    <div class="alert __alert-2 alert-warning m-0 py-1 px-2" role="alert">
        <img class="rounded mr-1" width="25" src="{{ asset('/public/assets/admin/img/header_warning.png') }}"
            alt="">
        <div class="cont">
            <h4 class="m-0">{{ translate('Attention_Please') }} </h4>
...
@if (
    $Payable_Balance == 1 &&
        ...
        $wallet?->balance < 0 &&
        $cash_in_hand_overflow_store_amount < $wallet?->collected_cash)
    <div class="alert __alert-2 alert-warning m-0 py-1 px-2" role="alert">
        <img class="mr-1" width="25" src="{{ asset('/public/assets/admin/img/header_warning.png') }}"
            alt="">
        <div class="cont">
            <h4 class="m-0">{{ translate('Attention_Please') }} </h4>
...
@if (!in_array($store_data->store_business_model, ['none', 'commission']) && !Request::is('vendor-panel/subscription/*'))
    ...
    ?>
    @if (
        $store_data?->store_sub?->is_trial == 0 &&
            $store_data?->store_sub?->expiry_date_parsed &&
            $store_data?->store_sub->expiry_date_parsed->subDays($subscription_deadline_warning_days)->isBefore(now()) &&
            ...
            <div>
                ...
                @else
                    <a href="{{ route('vendor.subscriptionackage.subscriberDetail', ['renew_now' => true]) }}"
                        class="btn btn--danger">{{ translate('Renew') }}</a>
                @endif
                <button data-id="subscription_renew_close_btn" id="hide-warning"
    ...
    @if (Session::get('subscription_free_trial_close_btn') !== true &&
            $store_data?->store_sub?->status == 1 &&
            $store_data?->store_sub?->is_trial == 1 &&
            $store_data?->store_sub?->is_canceled == 0)
                    ...
                    <a href="#" class="btn btn-2">
                        <span class="circle-progress-container">
                            <svg width="40" viewBox="0 0 160 160">
                                <circle r="70" cx="80" cy="80" fill="transparent" stroke="#ffffff20"
                        ...
                        <div class="trial-ended-modal-wrapper">
                            <div class="trial-ended-modal-content align-self-center">
                                ...
                                <div class="blocked-subscription mt-5">
                                    <img src="{{ asset('/public/assets/admin/img/WarningOctagon.svg') }}"
                                        alt="">
    ...
    @elseif (Session::get('subscription_cancel_close_btn') !== true &&
            $store_data?->store_sub &&
            $store_data?->store_sub?->is_canceled == 1)
        <div class="free-trial trial danger-bg">
            <div class="inner-div">
                <div class="left">
                    <img src="{{ asset('/public/assets/admin/img/timer-2.svg') }}" alt="">
                    <div class="left-content">
                        <h6>{{ translate('Your_Subscription_Has_Been_Cnaceled_by') }}
                            {{ $store_data?->store_sub?->canceled_by == 'admin' ? translate($store_data?->store_sub?->canceled_by) : translate('Yourself') }}
                        </h6>
                        <div>{{ translate('You_can_not_consume_your_subscription_after') }}
                            {{ \App\CentralLogics\Helpers::date_format($store_data?->store_sub?->expiry_date_parsed) }}
                        </div>
                    </div>
                </div>
                <div class="right">
    ...
    @elseif (Session::get('subscription_plan_update_close_btn') !== true &&
            $store_data?->store_sub &&
            $store_data?->store_sub?->package?->status != 1)
        <div class="free-trial trial danger-bg">
            <div class="inner-div">
                <div class="left">
                    <img src="{{ asset('/public/assets/admin/img/timer-2.svg') }}" alt="">
                ...
                <div class="left">
                    ...
                    <div class="left-content">
                        <h6>{{ translate('Your_Subscription_Has_Been_Expired_on') }}
                            {{ \App\CentralLogics\Helpers::date_format($store_data?->store_sub_update_application?->expiry_date_parsed) }}
                        </h6>
                        <div>
                            {{ translate('Purchase a subscription plan or contact with the admin to settle the payment and unblock the access to service') }}
                        </div>
