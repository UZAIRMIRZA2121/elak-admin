...
<head>
    ...
    <style>
        ...
        .voucher-card:hover,
        .voucher-card_2:hover {
            transform: translateY(-2px);
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.15);
        }

        .voucher-card.selected,
        .voucher-card_2.selected {
...
<body class="footer-offset">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div id="starRating" class="d-flex gap-1">
                            <i class="tio-star star" data-value="1"></i>
                            <i class="tio-star star" data-value="2"></i>
                            <i class="tio-star star" data-value="3"></i>
                            <i class="tio-star star" data-value="4"></i>
                            <i class="tio-star star" data-value="5"></i>
                        </div>
                ...
                <div class="modal-footer">
                    <button class="btn btn-primary" id="updateReviewBtn">Update</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        messaging.onMessage(function(payload) {
            ...
            if (payload.data.order_id && payload.data.type == "order_request") {
                ...
                @if (
                    \App\CentralLogics\Helpers::module_permission_check('order') &&
                        $admin_order_notification &&
                        $order_notification_type == 'firebase')
                        ...
                        success: function(response) {
                            if (response.length === 0) {
                                ...
                                @if (!$current_module_type_for_search)
                                    htmlContent +=
                                        '<div class="bg--13 d-inline-block fs-12 fw-500 mb-2 px-2 py-1 rounded text-italic">' +
                                        @json(translate('* To get module-specific results, please search within the module.')) + '</div>';
                                @endif

                                htmlContent += '<div class="fs-16 fw-500 mb-2">' +
                                    @json(translate('Search Result')) + '</div>' +
                                    '<div class="search-list h-300 d-flex flex-column gap-2 justify-content-center align-items-center fs-16">' +
                                    '<img width="30" class="h-auto" src="' +
                                    @json(asset('/public/assets/admin/img/modal/no-search-found.png')) + '" alt="">' + ' ' +
                                ...
                                response.forEach(function(route) {
                                    var separator = route.fullRoute.includes('?') ?
                                        '&' : '?';
                                    var fullRouteWithKeyword = route.fullRoute +
                                        separator + 'keyword=' + encodeURIComponent(
                                            searchKeyword);
                                    var keywordRegex = searchKeyword.replace(
                                        /[.*+?^${}()|[\]\\]/g, '\\$&');
                                    keywordRegex = new RegExp('(' + keywordRegex + ')',
                                        'gi');
                                    var highlightedRouteName = route.routeName.replace(
                                        keywordRegex, '<mark>$1</mark>');
                                    var highlightedURI = route.URI.replace(keywordRegex,
                                        '<mark>$1</mark>');
                                    resultHtml += '<a href="' + fullRouteWithKeyword +
                                        '" class="search-list-item d-flex flex-column" data-route-name="' +
                                        route.routeName + '" data-route-uri="' + route
                                        .URI + '" data-route-full-url="' + route
                                        .fullRoute + '" aria-current="true">';
                                    resultHtml += '<h5>' + highlightedRouteName +
                                        '</h5>';
                                    resultHtml += '<p class="text-muted fs-12 mb-0">' +
                                        highlightedURI + '</p>';
                                    resultHtml += '</a>';
                                });
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.length === 0) {
                        ...
                        var resultHtml = '';
                        response.forEach(function(route) {
                            resultHtml += '<a href="' + route.route_full_url +
                                '" class="search-list-item d-flex flex-column" data-route-name="' +
                                route.route_name + '" data-route-uri="' + route.route_uri +
                                '" data-route-full-url="' + route.route_full_url +
                                '" aria-current="true">';
                            resultHtml += '<h5>' + route.route_name + '</h5>';
                            resultHtml += '<p class="text-muted fs-12  mb-0">' + route.route_uri +
                                '</p>';
                            resultHtml += '</a>';
                        });
                        $('#searchResults').html('<div class="recent-search fs-16 fw-500 animate">' +
                            @json(translate('Recent Search')) +
                            '<div class="search-list d-flex flex-column mt-2">' + resultHtml +
                            '</div></div>');
    ...
    let rating = $('#review_rating').val();
    let comment = $('#review_comment').val();

    btn.prop('disabled', true).text('Updating...');

    $.ajax({
        url: "{{ route('admin.Voucher.review.update') }}",
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
            id: id,
        ...
        success: function(response) {
            if (response.success) {
                ...
                $('#review-row-' + id + ' .review-rating').text(rating);

                let shortComment = comment.length > 30
                    ? comment.substring(0, 30) + '...'
                    : comment;

                $('#review-row-' + id + ' .review-comment').text(shortComment);

                let btnEdit = $('#review-row-' + id + ' .edit-review-btn');
                btnEdit.attr('data-rating', rating);
                btnEdit.attr('data-comment', comment);
