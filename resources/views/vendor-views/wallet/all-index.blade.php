...
@section('content')
    <div class="content container-fluid">
        ...
        @include('vendor-views.wallet.partials._all_balance_data', [
            'wallets' => $wallets,
            'vendorIds' => $vendorIds,
        ])
                        ...
                        @foreach ($withdraw_req as $k => $wr)
                            <tr>
                                                ...
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="exampleModalLabel">
                                                            {{ translate('messages.disbursement_method_details') }} </h5>
                                ...
                                <td>
                                    ...
                                    @elseif($wr->approved == 2)
                                        <label
                                            class="badge badge-soft-warning">{{ translate('messages.under_review') }}</label>
                                    @else
                                ...
                                <td>
                                    ...
                                    @if ($wr->approved == 0)
                                        {{-- <a class="btn btn-outline-danger btn--danger action-btn form-alert"
                                            href="javascript:" data-id="withdraw-{{ $wr['id'] }}"
                                    ...
                                    @elseif($wr->approved == 2)
                                        <a class="btn btn-outline-warning btn--warning "
                                            href="{{ route('vendor.wallet.under-review-completed', [$wr['id']]) }}"
                                            title="{{ translate('messages.review_completed') }}"> <i class="tio-done"></i>
                                        </a>
                                    @else
                            ...
                            @forelse ($data as $item)
                                <div class="col-sm-6">
                                    <div class="d-flex gap-3 align-items-center">
                                        <input type="radio" required id="{{ $item['gateway'] }}"
                                            name="payment_gateway" value="{{ $item['gateway'] }}">
    ...
    <div class="modal fade" id="Adjust_wallet" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('vendor.wallet.make_wallet_adjustment') }}" method="POST"
