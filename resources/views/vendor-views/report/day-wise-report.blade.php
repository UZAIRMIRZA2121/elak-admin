        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                <span>
                    ...
                    @if ($from && $to)
                        <span class="mb-0 h6 badge badge-soft-success ml-2" id="itemCount">( {{ $from }} -
                            {{ $to }} )</span>
                    @endif
                </span>
            </h1>
                ...
                <form>
                    ...
                    <div class="row g-3">
                        ...
                        @php
                            ...
                            $stores = \App\Models\Store::where('id', $store_id)
                            ->orWhere('parent_id', $store_id)
                            ->orderBy('parent_id', 'asc') // parent first
                        ...
                        <div class="col-sm-6 col-md-3">
                            <select name="store_id" data-url="{{ url()->full() }}" data-filter="store_id"
                                ...
                                class=" form-control set-filter">
                                @foreach ($stores as $child)
                                    <option value="{{ $child->id }}"
                                        {{ isset($store) && $store->id == $child->id ? 'selected' : '' }}> ─ {{ $child->name }} {{ $child->parent_id == null ? '( Main )' : '' }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        @if (isset($filter) && $filter == 'custom')
                            ...
                            <div class="col-sm-6 col-md-3">
                                ...
                                <input type="date" name="to" id="to_date" class="form-control"
                                    placeholder="{{ translate('End Date') }}" value="{{ $to ?? '' }}" required>
