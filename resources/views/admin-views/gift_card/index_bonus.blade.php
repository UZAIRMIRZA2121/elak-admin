        ...
        <!-- End Page Header -->
            ...
            <div id="manage" class="section">
                ...
                <div class="conditions-grid">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                        <form action="{{route('admin.Giftcard.list_bonus')}}" method="post" enctype="multipart/form-data" id="occasionForm">
                              @csrf
                              @method("post")
                            <div id="list-occasions" class="section">
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="form-group">
                                        <label for="search_store">Filter by Store </label>
                                        <select id="search_store" name="search_store" >
                            ...
                            <div id="occasionsList" class="row">
                                 @foreach($BonuLimitSetting as $key => $UsageTerm)
                                    <div class="col-md-4 mb-3 ">
                                        <div class="card shadow-sm h-100">
                                         ...
                                         <span class="badge status-badge text-capitalize text-white {{ $UsageTerm->status == 'active' ? 'bg-success' : 'bg-danger' }}"
                                            ...
                                            <p class="mb-1"><strong>Voucher Type:</strong> {{ $UsageTerm->voucher_type_name }}</p>
                                            <p class="mb-0"><strong>Minimum Gift Card Amount:</strong> {{ $UsageTerm->min_gift_ard }}</p>
                                            <p class="mb-0"><strong>Maximum Gift Card Amount:</strong> {{ $UsageTerm->max_gift_ard }}</p>
                                            <p class="mb-0"><strong>Store Name:</strong> {{ $UsageTerm->store_name }}</p>
                                            <p class="mb-0"><strong>Created:</strong> {{ $UsageTerm->created_at }}</p>
                                                @php(

                                                    $config = json_decode($UsageTerm->multi_level_bonus_configuration, true)
                                                )


                                                <p class="mb-0"><strong>Multi-Level Bonus Configuration:</strong></p>
                                                <table class="table table-bordered mt-2">
                                                    ...
                                                    <tbody>
                                                        @foreach($config['min'] as $index => $min)
                                                            <tr>
                                                                <td>{{ $index + 1 }}</td>
                                                                <td>{{ $min }}</td>
                                                                <td>{{ $config['max'][$index] ?? '' }}</td>
                                                                <td>{{ $config['bonus'][$index] ?? '' }}</td>
                                                            </tr>
                                                        @endforeach
                                                    </tbody>
                                                </table>
                                            ...
                                            <div class="card-footer d-flex justify-content-end">
                                                ...
                                                href="{{ route('admin.Giftcard.edit_bonus', [$UsageTerm->id]) }}"
                                                title="Edit">
                                                    Edit
                                                </a>
                                                ...
                                                <form action="{{ route('admin.Giftcard.delete_bonus', [$UsageTerm->id]) }}"
                    ...
                    @if(count($BonuLimitSetting) !== 0)
                    <hr>
                    @endif
                    <div class="page-area">
                        {!! $BonuLimitSetting->links() !!}
                    </div>
                    @if(count($BonuLimitSetting) === 0)
...
$(document).on("click", ".toggle-status-btn", function () {
    ...
    $.ajax({
        url: "/admin/Giftcard/status-toggle-bonus/" + id,
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}"
        },
