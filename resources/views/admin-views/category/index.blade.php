        ...
        <div class="card-body">
            <form
                ...
                <ul class="nav nav-tabs mb-4 border-0">
                    <li class="nav-item">
                        <a class="nav-link lang_link active" href="#" id="default-link">{{ translate('messages.default')
                            }}</a>
                    </li>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="form-group d-none lang_form" id="{{ $lang }}-form">
                            ...
                            <input type="text" name="name[]" value="{{ old('name.' . $key + 1) }}" class="form-control"
                                placeholder="{{ translate('messages.new_category') }}" maxlength="191">
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group d-flex align-items-center mb-3">
                                ...
                                <div class="form-check form-switch">
                                    <input type="checkbox" class="form-check-input" id="scheduleToggle"
                        ...
                        <div class="form-group mt-3">
                            <label class="input-label" for="zone_ids">{{ translate('messages.zone') }}</label>
                            <select name="zone_ids[]" id="zone_ids" class="form-control js-select2-custom"
                    ...
                    <div class="col-md-6">
                        <div class="h-100 d-flex align-items-center flex-column">
                            ...
                            <label class="text-center my-auto position-relative d-inline-block">
                                <img class="img--176 border" id="viewer" @if (isset($category))
                                    src="{{ asset('storage/app/public/category') }}/{{ $category['image'] }}" @else
                                    src="{{ asset('public/assets/admin/img/upload-img.png') }}" @endif alt="image" />
                ...
                <div class="btn--container justify-content-end mt-3">
                    ...
                    <button type="submit" class="btn btn--primary">{{ isset($category) ? translate('messages.update') :
    ...
    <div class="card mt-3">
        <div class="card-header py-2 border-0">
            <div class="search--button-wrapper">
                <h5 class="card-title">{{ translate('messages.category_list') }}<span class="badge badge-soft-dark ml-2"
                ...
                <table id="columnSearchDatatable" class="table table-borderless table-thead-bordered table-align-middle"
                    ...
                    <tbody id="table-div">
                        ...
                        <tr>
                            ...
                            <td>
                                <label class="toggle-switch toggle-switch-sm" for="featuredCheckbox{{ $category->id }}">
                                    <input type="checkbox"
                                        ...
                                        id="featuredCheckbox{{ $category->id }}" {{ $category->featured ? 'checked' : ''
                                    }}>
                                    <span class="toggle-switch-label mx-auto">
                                        <span class="toggle-switch-indicator"></span>
                                    </span>
                            ...
                            <td>
                                <span class="d-block font-size-sm text-body">
                                    @forelse ($category?->taxVats?->pluck('tax.name', 'tax.tax_rate')->toArray() as $key
                                    => $tax)
                            ...
                            <td>
                                @php($category_zones = $category->zones)
                                @if(count($category_zones) > 0)
                                @foreach($category_zones as $zone)
                                <span class="badge badge-soft-info">{{ $zone->name }}</span>
                                @endforeach
                                @else
                                <span class="badge badge-soft-secondary">{{ translate('messages.all') }}</span>
                                @endif
                            </td>
                            ...
                            <td>
                                ...
                                <form action="{{ route('admin.category.priority', $category->id) }}"
                                    ...
                                    <select name="priority" id="priority"
                                        ...
                                        <option value="0" class="text--title" {{ $category->priority == 0 ? 'selected' :
                                            '' }}>
                                            {{ translate('messages.normal') }}</option>
                                        <option value="1" class="text--title" {{ $category->priority == 1 ? 'selected' :
                                            '' }}>
                                            {{ translate('messages.medium') }}</option>
                                        <option value="2" class="text--title" {{ $category->priority == 2 ? 'selected' :
                                            '' }}>
