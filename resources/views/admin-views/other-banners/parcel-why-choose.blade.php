            ...
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        @endif
                        <form action="{{ route('admin.promotional-banner.why-choose-store') }}" method="POST"
                                                ...
                                                <div class="form-group">
                                                    <label class="form-label">{{ translate('messages.Short_Description') }}
                                                        ({{ translate('messages.default') }})<span
                                                            class="form-label-secondary" data-toggle="tooltip"
                                                            data-placement="right"
                                                            data-original-title="{{ translate('Write_the_short_description_within_100_characters') }}">
                                                            <img src="{{ asset('public/assets/admin/img/info-circle.svg') }}"
                                                                alt="">
                                                        </span></label>
                                                    <textarea type="text" maxlength="100" name="short_description[]" class="form-control" rows="3"
                                                        {{ translate('messages.short_description_here...') }}> </textarea>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        @foreach (json_decode($language) as $lang)
                                            <div class="row d-none lang_form" id="{{ $lang }}-form1">
                                                ...
                                                <div class="col-12">
                                                    <div class="form-group">
                                                        <label
                                                            class="form-label">{{ translate('messages.Short_Description') }}
                                                            ({{ strtoupper($lang) }})<span class="form-label-secondary"
                                                                data-toggle="tooltip" data-placement="right"
                                                                data-original-title="{{ translate('Write_the_short_description_within_100_characters') }}">
                                                                <img src="{{ asset('public/assets/admin/img/info-circle.svg') }}"
                                    ...
                                    @foreach ($banners as $key => $banner)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <form
                                                    action="{{ route('admin.promotional-banner.why-choose-status-update', [$banner->id, $banner->status ? 0 : 1]) }}"
                                            ...
                                            <td>
                                                <div class="btn--container justify-content-center">
                                                    <a class="btn action-btn btn--primary btn-outline-primary"
                                                        href="{{ route('admin.promotional-banner.why-choose-edit', [$banner['id']]) }}">
                                                        <i class="tio-edit"></i>
                                                    </a>
                                                    <a class="btn action-btn btn--danger btn-outline-danger form-alert-title"
                                                        href="javascript:" data-id="criteria-{{ $banner['id'] }}"
                                                        data-title="{{ translate('Want_to_delete_this_feature_?') }}"
                                                        data-message="{{ translate('If_yes,_It_will_be_removed_from_this_list_and_this_module.') }}"
                                                        title="{{ translate('messages.delete_criteria') }}"><i
                                                            class="tio-delete-outlined"></i>
                                                    </a>
                                                    <form
                                                        action="{{ route('admin.promotional-banner.why-choose-delete', [$banner['id']]) }}"
                                                        method="post" id="criteria-{{ $banner['id'] }}">
                                                        @csrf @method('delete')
                                                    </form>
                                                </div>
                                            </td>
