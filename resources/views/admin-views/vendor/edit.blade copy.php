                ...
                <div class="card shadow--card-2">
                    <div class="card-body">
                        ...
                        <ul class="nav nav-tabs mb-4">
                            <li class="nav-item">
                                <a class="nav-link lang_link active" href="#" id="default-link">{{ translate('Default')
                                    }}</a>
                            </li>
                        ...
                        <div class="lang_form" id="default-form">
                            <div class="form-group">
                                ...
                                <input type="text" name="name[]" id="default_name" class="form-control"
                                    placeholder="{{ translate('messages.store_name') }}" value="{{ $store->name }}"
                                    required>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="d-flex flex-wrap flex-sm-nowrap __gap-12px">
                            ...
                            <div class="__custom-upload-img">
                                ...
                                @php($icon = $icon->value ?? '')
                                <label class="form-label">
                                    {{ translate('Store Cover') }} <span class="text--primary">({{ translate('2:1')
                                        }})</span>
                                </label>
                                <label class="text-center position-relative">
                        ...
                        <div class="row g-3 my-0">
                            <div class="col-md-12">
                                <div class="position-relative">
                                    <label class="input-label" for="tax">{{ translate('Estimated Delivery Time ( Min &
                                        Maximum Time)') }}</label>
                                        ...
                                        <div class="card shadow--card-2">
                                            <div class="card-body">
                                                <div class="floating--date-inner">
                                                    <div class="item">
                                                        <label class="input-label" for="minimum_delivery_time">{{
                                                            translate('Minimum Time') }}</label>
                                                    ...
                                                    <div class="item">
                                                        <label class="input-label" for="maximum_delivery_time">{{
                                                            translate('Maximum Time') }}</label>
                                                    ...
                                                    <div class="item smaller">
                                                        <select name="delivery_time_type" id="delivery_time_type"
                                                            class="custom-select">
                                                            <option value="min" {{ $delivery_time_type=='min'
                                                                ? 'selected' : '' }}>
                                                                {{ translate('messages.minutes') }}</option>
                                                            <option value="hours" {{ $delivery_time_type=='hours'
                                                                ? 'selected' : '' }}>
                                                                {{ translate('messages.hours') }}</option>
                                                            <option value="days" {{ $delivery_time_type=='days'
                                                                ? 'selected' : '' }}>
                                                                {{ translate('messages.days') }}</option>
                                                        </select>
                                                    </div>
                                                    <div class="item smaller">
                                                        <button type="button" class="btn btn--primary delivery-time">{{
                            ...
                            <div class="col-lg-4">
                                ...
                                <div class="form-group mb-5">
                                    ...
                                    <label class="input-label" for="type">
                                        ...
                                        <input type="checkbox" name="type" id="type" class="mt-2" value="1" {{
                                ...
                                <div class="form-group" id="sub_branch_group">
                                    ...
                                    <select name="parent_id" id="parent_id" class="form-control js-select2-custom"
                                        ...
                                        @php($main_stores = \App\Models\Store::active()->where('type', 'main')->get())
                                        @foreach ($main_stores as $Store_item)
                                        @if (isset(auth('admin')->user()->Store_id))
                                        @if (auth('admin')->user()->Store_id == $Store_item->id)
                                        <option value="{{ $Store_item->id }}" {{ $store->parent_id==$Store_item->id ?
                                            'selected' : '' }}>
                                ...
                                {{-- <div class="form-group">
                                    ...
                                    <select name="voucher_id[]" id="voucher_id" required
                                        ...
                                        <option value="{{ $VoucherType->id }}" @if ((old('voucher_id') &&
                                            in_array($VoucherType->id, old('voucher_id'))) ||
                                            (isset($selectedVoucherIds) && in_array($VoucherType->id,
                                            $selectedVoucherIds)))
                                            selected
                                            @endif
...
?>
                                <div class="form-group">
                                    ...
                                    <select name="category_id[]" id="category_id" required
                                        ...
                                        <option value="{{ $VoucherType->id }}" @if ((old('category_id') &&
                                            in_array($VoucherType->id, old('category_id'))) ||
                                            (isset($selectedVoucherIds) && in_array($VoucherType->id,
                                            $selectedVoucherIds))) selected @endif>
                                            {{ $VoucherType->name }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-lg-8">
                                ...
                                <div class="form-group " style="margin-top: 92px">
                                    ...
                                    <input type="text" id="bonus_tiers" value="{{$store->bonus_tiers}}"
                                        name="bonus_tiers" class="form-control" placeholder="Bonus Tiers" required>
                                </div>

                                <div class="form-group">
                                    <label class="input-label" for="limit_to">{{translate('Limit To')}}</label>
                                    <input type="text" id="limit_to" value="{{$store->limit_to}}" name="limit_to"
                                        class="form-control" placeholder="Limit To" required>
                                </div>
                                <div class="form-group">
                                    <label class="input-label" for="flate_discount">{{translate('Flate
                                        Discount')}}</label>
                                    <input type="text" id="flate_discount" value="{{$store->flate_discount}}"
                                        name="flate_discount" class="form-control" placeholder="Flate Discount"
                                        required>
                                </div>
                            </div>
                        </div>
...
?>
                            ...
                            <div class="staff-item mb-3" data-index="{{ $index }}">
                                <div class="row g-2 align-items-center">
                                    <div class="col-md-4">
                                        <input type="text" name="staff[{{ $index }}][name]" class="form-control"
                                            placeholder="Staff Name" value="{{ $staff['name'] ?? '' }}">
                                    </div>
                                    <div class="col-md-4">
                                        <input type="text" name="staff[{{ $index }}][role]" class="form-control"
                                            placeholder="Staff Role" value="{{ $staff['role'] ?? '' }}">
                                    </div>
                                    <div class="col-md-3">
                                        <input type="text" name="staff[{{ $index }}][phone]" class="form-control"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4 col-sm-6">
                                <div class="js-form-message form-group mb-0">
                                    <label class="input-label" for="signupSrPassword">{{ translate('password') }}<span
                                            class="form-label-secondary" data-toggle="tooltip" data-placement="right"
                            ...
                            <div class="col-md-4 col-sm-6">
                                <div class="js-form-message form-group mb-0">
                                    <label class="input-label" for="signupSrConfirmPassword">{{
                                        translate('messages.Confirm Password') }}</label>
...
?>
                            ...
                            <div class="file-box" id="file-{{ $index }}">
                                ...
                                <div class="pdf-clickable clickable-file"
                                    ...
                                    <embed src="{{ asset('storage/store/' . $file) }}#toolbar=0&navpanes=0&scrollbar=0"
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-body p-0 text-center">
                                            ...
                                            <iframe id="previewPDF" src=""
                                                style="width:100%; height:80vh; display:none;" frameborder="0"></iframe>
                                        </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#doc_edit_btn').on('click', function () {
            $('#doc-upload-wrapper').toggle();
        });

        $('#doc-upload-wrapper').on('click', function () {
            $(this).find('input[type="file"]').trigger('click');
        });

        previewFile('.document_input', '#logoImageViewer2', '.textbox');
    });
</script>