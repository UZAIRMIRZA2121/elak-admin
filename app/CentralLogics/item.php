...
class ProductLogic
{
    ...
    public static function get_latest_products($zone_id, $limit, $offset, $store_id = null, $category_id = null, $type, $min = false, $max = false, $product_id = null, $filter = null, $rating_count = null)
    {
        ...
        $query = Item::
            when(!is_null($category_id), function ($q) use ($category_id) {
                $q->whereHas('category', function ($q) use ($category_id) {
                    $q->whereIn('id', $category_id)
                        ->orWhereIn('parent_id', $category_id);
                });
            })
            ...
            ->when(!is_null($store_id), function ($query) use ($store_id) {

                $query->when(is_numeric($store_id), function ($q) use ($store_id) {
                    $q->where('store_id', $store_id);
                });

                $query->when(!is_numeric($store_id), function ($q) use ($store_id) {
                    $q->whereHas('store', function ($s) use ($store_id) {
                        $s->where('slug', $store_id);
                    });
                });
    ...
    public static function recommended_items($zone_id, $store_id = null, $limit = null, $offset = null, $type = 'all', $filter = 'all')
    {
        ...
        if ($limit != null && $offset != null) {
            ...
            $paginator = Item::    where('type', 'voucher')
