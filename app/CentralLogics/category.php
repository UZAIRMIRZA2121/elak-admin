...
class CategoryLogic
{
    ...
    public static function category_stores($category_ids, $zone_id, int $limit,int $offset, $type,$longitude=0,$latitude=0,$filter=null,$rating_count=null)
    {
      ...
      public static function stores_all_gift(
        $category_id,
        $zone_id,
        int $limit,
        int $offset,
        ...
        // dd($category_id);
        \Illuminate\Support\Facades\Log::info("CategoryLogic stores category_id: " . json_encode($category_id));

        $paginator = Store::withOpen($longitude ?? 0, $latitude ?? 0)
            // ✅ Load FULL voucher data

            ->whereHas('items', function ($q) {
                $q->where('voucher_ids', 'Gift');
            })
           ...
           ->with([
            ...
            ->when($category_id != 'all' && $category_id != 0, function($query)use($category_id){
