...
{
    $category_sub_category_item_default_status =
        BusinessSetting::where('key', 'category_sub_category_item_default_status')->first()?->value ?? 1;

    $category_sub_category_item_sort_by_general =
        PriorityList::where('name', 'category_sub_category_item_sort_by_general')
            ->where('type', 'general')->first()?->value ?? '';

    $category_sub_category_item_sort_by_unavailable =
        PriorityList::where('name', 'category_sub_category_item_sort_by_unavailable')
            ->where('type', 'unavailable')->first()?->value ?? '';

    $category_sub_category_item_sort_by_temp_closed =
        PriorityList::where('name', 'category_sub_category_item_sort_by_temp_closed')
            ->where('type', 'temp_closed')->first()?->value ?? '';

    /* ------------------------------------
     | Normalize zone_id ONCE (CRITICAL)
     |------------------------------------ */
    if (is_string($zone_id)) {
        $zone_id = json_decode($zone_id, true);
    }

    $zone_id = is_array($zone_id) ? array_filter($zone_id) : [$zone_id];
    ...
    if ($category_sub_category_item_default_status == '1') {
        ...
        match ($category_sub_category_item_sort_by_general) {
