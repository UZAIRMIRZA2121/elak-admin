...
class LoginController extends Controller
{
    ...
    {
        $this->middleware('guest:admin,vendor,client', ['except' => 'logout']);
    }
    ...
    public function login($login_url)
    {
        ...
        $data = array_column(DataSetting::whereIn('key', ['store_employee_login_url', 'store_login_url', 'admin_employee_login_url', 'admin_login_url', 'client_login_url','client_employee_login_url'
        ...
        $loginTypes = [
            ...
            'client' => 'client_login_url',
            'client_employee' => 'client_employee_login_url',
        ...
        $siteDirections = [
            ...
            'vendor_employee' => session()?->get('vendor_site_direction') ?? $direction ?? 'ltr',
             'client' => session()?->get('client_site_direction') ?? $direction ?? 'ltr',
            'client_employee' => session()?->get('client_site_direction') ?? $direction ?? 'ltr',
        ];
        $locals = [
            ...
            'client' => session()?->get('client_local') ?? $lang ?? 'en' ,
            'client_employee' => session()?->get('client_local') ?? $lang ?? 'en',
    ...
    public function submit(Request $request)
    {
        ...
        elseif ($request->role == 'admin') {
         ...
         $data = Client::where('email', $request->email)->where('status', "active")->exists();
        ...
        if ($data == 'client') {
            ...
            // if (isset($modules) && ($modules->count() > 0)) {
                ...
                return redirect()->route('client.dashboard')->withCookies($forgetCookies);
            // }
